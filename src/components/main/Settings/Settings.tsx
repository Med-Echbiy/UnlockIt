...
import { WallpaperSelector } from "../Game/sections/WallpaperSelector";
import { CoverSelector } from "../Game/sections/CoverSelector";
...
function SettingsDialog() {
  ...
  const [selectedGame, setSelectedGame] = useState<{
    ...
    name: string;
    header_image: string;
    library_cover: string | null;
  } | null>(null);
  const dialogRef = useRef<HTMLDivElement>(null);
  const { open, toggle } = settingsModalStore();
  const { games } = useMyGamesStore();
  const { setGameHeaderImage, setGameLibraryCover } = useUpdateGameWorkflow();
  const {
    getProfile,
    updateName,
    updateAvatar,
  ...
  const handleWallpaperSelected = async (imagePath: string) => {
    if (selectedGame) {
      await setGameHeaderImage(selectedGame.appId, imagePath);
  ...
  const handleCoverSelected = async (imagePath: string) => {
    if (selectedGame) {
      await setGameLibraryCover(selectedGame.appId, imagePath);
  ...
  return (
    <>
          ...
          >
            ...
            <div className='px-6 pb-6 pt-2 space-y-8'>
              ...
              <div className='space-y-6'>
                ...
                <div className='space-y-4'>
                  <Label>Select a game to change its images</Label>
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (game) {
                        setSelectedGame({
                          ...
                          library_cover: game.library_cover || null,
                        });
                      }
                    }}
                  ...
                  {selectedGame && (
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-3'>
                      <Button
                        onClick={() => {
                          toggle(); // Close settings dialog
                          setWallpaperDialogOpen(true); // Open wallpaper dialog
                        }}
                        variant='outline'
                        className='flex items-center gap-2'
                      >
                      ...
                      <Button
                        onClick={() => {
                          toggle(); // Close settings dialog
                          setCoverDialogOpen(true); // Open cover dialog
                        }}
                        variant='outline'
                        className='flex items-center gap-2'
                      >
      ...
      {/* Wallpaper Selector Dialog - Separate root level */}
      {selectedGame && (
        <WallpaperSelector
          ...
          currentImagePath={selectedGame.header_image}
          onWallpaperSelected={handleWallpaperSelected}
        />
      )}

      {/* Cover Selector Dialog - Separate root level */}
      {selectedGame && (
        <CoverSelector
          ...
          gameName={selectedGame.name}
          appId={selectedGame.appId}
          currentImagePath={selectedGame.library_cover}
